// page_index.mjs — Dashboard: request expenses + show debts
...
async function firebaseReady(){
  auth.onAuthStateChanged(async (user)=>{
    ...
    function renderList(el, map){
      ...
      el.innerHTML = arr.length ? arr.map(([k,v])=>`<div class="d-flex justify-content-between"><span>${k}</span><span>${fmt.format(v)}</span></div>`).join("") : "<div class='text-muted'>—</div>";
    }
    const q1 = query(collection(db,"splits"), where("status","==","approved"), where("monthKey","==", mk), where("payerId","==", user.uid));
    onSnapshot(q1, (snap)=>{
      const m={}; snap.forEach(d=>{ const x=d.data(); const name=x.debtorName||x.debtorId; m[name]=(m[name]||0)+Number(x.shareAmount||0); }); renderList(box1, m);
    });
    const q2 = query(collection(db,"splits"), where("status","==","approved"), where("monthKey","==", mk), where("debtorId","==", user.uid));
    onSnapshot(q2, (snap)=>{
      const m={}; snap.forEach(d=>{ const x=d.data(); const name=x.payerName||x.payerId; m[name]=(m[name]||0)+Number(x.shareAmount||0); }); renderList(box2, m);
    });
  });
}
